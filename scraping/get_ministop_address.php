...
for($i=8;$i<=47;$i++){
	$listPrefSeven = array();
	$key = sprintf("%02d",$i);
	$search_url = "http://www.e-map.ne.jp/mobile/ministop/cl.htm?p=s&slg=1&key=".$key."&opt=COL_13=1@COL_14=1@COL_14=2&";
	do{
print $search_url."\n";
		$search_source = getAccessSource($CON_USLEEP,$search_url);
		$search_source = mb_convert_encoding($search_source,"UTF-8","SJIS");
		if( preg_match_all("/<a\s+?href=[\"'](cl\.htm\?p=.*?&slg=[0-9]*?&tod=.*?&shk=.*?&opt=COL_13=[0-9]*?@COL_14=[0-9]*?@COL_14=[0-9]*?&)[\"'][\s\S.]*?>([\s\S.]*?)<\/a>/im",$search_source,$listPref) > 0 ){
			foreach($listPref[1] as $index => $pref_url){
				$listPrefSeven[] = array($pref_url,$listPref[2][$index]);
			}
		}
		$next_flg = 'ok';
		if( preg_match_all("/(.*?前ページ<\/a>／)?<A\s+?href\s*?=\s*?[\"'](cl\.htm\?p=.*?&slg=[0-9]*?&key=[0-9]*?&pg=[0-9]*?&tod=.*?&shk=.*?&opt=COL_13=[0-9]*?@COL_14=[0-9]*?@COL_14=[0-9]*?&)[\"']>次ページ<\/a>/u",$search_source,$listPref) > 0 ){
			$search_url = "http://www.e-map.ne.jp/mobile/ministop/".$listPref[2][0];
			$next_flg = 'ng';
		}
	}while( $next_flg == 'ng' );
...
usleep(250000);
	do{
		$search_url = "http://www.e-map.ne.jp/mobile/ministop/".$record[0];
print $search_url."\n";
		$search_source = getAccessSource($CON_USLEEP,$search_url);
		$search_source = mb_convert_encoding($search_source,"UTF-8","SJIS");
		//店舗一覧
		if( preg_match_all("/<a\s+?href=\"(d\.htm\?id=[0-9]*?&opt=COL_13=[0-9]*?@COL_14=[0-9]*?@COL_14=[0-9]*?)\"[\s\S.]*?>([\s\S.]*?)<\/a>/im",$search_source,$listPref) > 0 ){
			foreach($listPref[1] as $index => $pref_url){
				$listPrefSeven1[$record[1]][] = array($pref_url,$listPref[2][$index]);
			}
		}
		$next_flg = 'ok';
		if( preg_match_all("/(.*?前ページ<\/a>／)?<A\s+?href\s*?=\s*?[\"'](cl\.htm\?p=.*?&slg=[0-9]*?&key=.*?&pg=[0-9]*?&tod=.*?&shk=.*?&opt=COL_13=[0-9]*?@COL_14=[0-9]*?@COL_14=[0-9]*?&)[\"'][\s\S.]*?>次ページ<\/a>/u",$search_source,$listPref) > 0 ){
...
print $search_url."\n";
		...
		$search_source = mb_convert_encoding($search_source,"UTF-8","SJIS");
		$listAddr = array();
		if( preg_match("/<h1[\s\S.]+?><span[\s\S.]+?>([\s\S.]+?)<\/span><\/h1>/im",$search_source,$listShop) > 0 ){
			$listAddr[] = $listShop[1];
		}
		if( preg_match("/<h4[\s\S.]*?><img\s+?src=\"[\s\S.]*?\"[\s\S.]*?\/><span[\s\S.]*?>住所<\/span><\/h4>[\s\S]*?<div[\s\S.]*?>([\s\S.]+?)<\/div>/im",$search_source,$listShop) > 0 ){
			$listAddr[] = $listShop[1];
		}
