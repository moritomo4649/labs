...
$genre_data = @file_get_contents("http://www.yaplog.jp/genre/");
$genre_data = mb_convert_encoding($genre_data,"UTF-8","SJIS");

preg_match_all("/<li><a\s+?href=\"(http:\/\/www\.yaplog\.jp\/genre\/.*?\/)\">(.*?)<\/a><\/li>/u",$genre_data,$listGenre);
...
print $genre."\n";
	...
	do{
		usleep(100000);
		$blog_data = @file_get_contents($genre.$listCount."/");
		$blog_data = mb_convert_encoding($blog_data,"UTF-8","SJIS");
print $genre.$listCount."/\n";
		$blog_data = preg_replace("/\n/u","",$blog_data);
		preg_match_all("/<a\s+?href=\"(http:\/\/yaplog\.jp\/.+?\/archive\/[0-9]+?)\"\s+?target=\"_blank\">(.*?)<\/a>/u",$blog_data,$matches);
		preg_match_all("/<p\s+?class=\"detail\">(.*?)<\/p>/u",$blog_data,$info);
		foreach($matches[1] as $index => $url){
			if( mb_eregi("{$join_str}",$info[1][$index]) ){
				fwrite($fp,trim($matches[2][$index]).",".$url."\n");
...
print "page:".$listCount."\n";
		if( $listCount >= $LIMIT )
			break;
		$listCount++;
		$genre_pettern = preg_replace("/\//u","\\/",$genre);
		$genre_pettern = preg_replace("/\./u","\\.",$genre_pettern);
		$err_flg = preg_match("/<a\s+?href=\"".$genre_pettern.$listCount."\/\">次へ<\/a>/u",$blog_data);
	}while( $err_flg > 0 );
}
fclose($fp);
