...
for($i=27;$i<=27;$i++){
	$listPrefSeven = array();
	$key = sprintf("%02d",$i);
	$search_url = "http://www.e-map.ne.jp/mobile/711map/cl.htm?p=s&slg=1&key=".$key."&&p_s1=00037&p_f2=1&p_f4=1&&optcd=1&sej_nwshp=";
	do{
print $search_url."\n";
		$search_source = getAccessSource($CON_USLEEP,$search_url);
		$search_source = mb_convert_encoding($search_source,"UTF-8","SJIS");
		if( preg_match_all("/<a\s+?href=[\"'](cl\.htm\?p=.*?&slg=[0-9]*?&tod=.*?&shk=.*?&&p_s1=[0-9]*?&p_f2=[0-9]*?&p_f4=[0-9]*?&&optcd=[0-9]*?&sej_nwshp=.*?)[\"'][\s\S.]*?>(.*?)<\/a>/im",$search_source,$listPref) > 0 ){
			foreach($listPref[1] as $index => $pref_url){
				$listPrefSeven[] = array($pref_url,$listPref[2][$index]);
			}
		}
		$next_flg = 'ok';
		if( preg_match_all("/(.*?前ﾍﾟｰｼﾞ<\/a>\/)?<A\s+?href\s*?=\s*?[\"'](cl\.htm\?p=.*?&slg=[0-9]*?&key=[0-9]*?&pg=[0-9]*?&tod=.*?&shk=.*?&&p_s1=[0-9]*?&p_f2=[0-9]*?&p_f4=[0-9]*?&&optcd=[0-9]*?)[\"']>次ﾍﾟｰｼﾞ<\/a>/u",$search_source,$listPref) > 0 ){
			$search_url = "http://www.e-map.ne.jp/mobile/711map/".$listPref[2][0];
			$next_flg = 'ng';
		}
	}while( $next_flg == 'ng' );
