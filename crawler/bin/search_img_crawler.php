...
$search_flg			 = 'start';
	...
	//リダイレクトしているかチェック
	$location_url = checkRedirectUrl($in_url,&$domain_type,&$search_host,&$exclusion_mode,&$id,&$firstAccessSource);
	...
	//検知先のURLリストを取得
	setDepthUrl($in_url,&$domain_type,&$search_host,$exclusion_mode,$search_max_depth,&$id,$badwords,&$firstAccessSource);

	$listTmpData = array();
	if( count($LIST_SEC_URL) > 0 || $search_max_depth == 1 ){
