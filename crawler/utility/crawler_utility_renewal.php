...
$LIMIT_MAIL_TIME		 = 120;
$CURL_CON_SLEEP			 = 1000000;
$MAX_DEPTH				 = 500;

//DB関連
$DB_JUDGE_HOST			 = 'localhost';
$DB_JUDGE_USER			 = 'crawler';
$DB_JUDGE_PASS			 = 'crawler5963';
$DB_JUDGE_NAME			 = 'crawler';
...
__END_SQL__;
	...
	function setHostPath($url){
		...
		if( $listCount > 0 ){
			...
			$listParam = preg_split("/\//",$listParam2[2]);
			$PATH = $listParam2[2];
		}else{
...
print $tmp_url.":フレームあり\n";
				...
				//初期化
				list($LIST_SEC_URL, $listDepthUrl) = setInitListDepthUrl($listTmpUrl,$LIST_SEC_URL,$listSaveUrl,$listDepthUrl,$id,$d,$domain_type,$exist_url,$exclusion_mode);
			}
		}
		$LIST_SEC_URL		 = array();
...
//		setListSearchUrl($id);
		$LIST_SEC_URL = array();
		$LIST_SEC_URL = setListExistSearchUrl($exist_url,$id,$domain_type,$depth);
		return $LIST_SEC_URL;
	}
...
*/
	...
	function setInitListDepthUrl($listTmpUrl,$LIST_SEC_URL,$listSaveUrl,$listDepthUrl,$id,$level,$domain_type,$exist_url,$exclusion_mode){
		global $listDebugUrl;
...
print_r($listDepthUrl);
print "test\n";
		return [$LIST_SEC_URL,$listDepthUrl];
	}
...
print $url."\n";
		...
		if( $err_flg !== false ){
			...
			if( !$first_asoruce ){
				return ['CURL_CON_ERR',$first_asoruce];
			}
			...
			foreach($listCurlData as $record){
				...
				if( !is_null($locationUrl) ){
					...
					return [true,$first_asoruce];
				}
			}
			//更にリダイレクトしていれば辿る
			if( preg_match("/^3/",$listInfo['http_code']) == 1 ){
				[$re_location_url,$first_asoruce] = checkRedirectUrl($locationUrl,$domain_type,$search_host,$exclusion_mode,$id,$first_asoruce);
				if( !is_null($re_location_url) ){
					...
					return [$re_location_url,$first_asoruce];
				}
				return [$locationUrl,$first_asoruce];
			}
			if( !is_null($locationUrl) ){
				return [$locationUrl,$first_asoruce];
			}
...
print "フレームURLがあった ==================================\n";
				...
				if( is_null($ratio_flg) ){
					//metaタグを除外する
					$meta_check_flg = regMetaLocateUrl($record,'',$domain_type,$search_host,$exclusion_mode,$id,$level);
					if( !$meta_check_flg )
						continue;
				}
				$html .= $record;
